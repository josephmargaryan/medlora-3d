    ...
    for batch in loader:
        ...
        y_onehot = (
            torch.nn.functional.one_hot(
                labels.long().squeeze(1), num_classes=int(logits.shape[1])
            )
            .permute(0, 4, 1, 2, 3)
            .float()
        )
        p_onehot = (
            torch.nn.functional.one_hot(
                preds.long().squeeze(1), num_classes=int(logits.shape[1])
            )
            .permute(0, 4, 1, 2, 3)
            .float()
        )
...
def train(
    ...
    stopper = (
        EarlyStopper(patience=patience, min_epochs=min_epochs, delta=0.0)
        if early_stopping
        else None
    )
