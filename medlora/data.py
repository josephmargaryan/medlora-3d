...
def is_ct(task): return task in CT_TASKS
...
def make_transforms(task, roi=DEFAULT_ROI, aug=True):
    ...
    norm = [ScaleIntensityRanged(keys=["image"], a_min=-1000, a_max=1000, b_min=0.0, b_max=1.0, clip=True)] if is_ct(task) \
           else [NormalizeIntensityd(keys=["image"], nonzero=True, channel_wise=True)]
    if aug:
        extra = [
            RandCropByPosNegLabeld(keys=["image","label"], label_key="label", spatial_size=roi, pos=1, neg=1, num_samples=1),
...
def load_decathlon_list(data_dir: Path, task: str, download=True):
    ds = DecathlonDataset(root_dir=str(data_dir), task=task, section="training",
                          transform=None, download=download, val_frac=0.0,
...
def take_fraction(items, frac_percent, seed):
    if frac_percent >= 100: return items
    n = max(1, int(len(items) * (frac_percent/100.0)))
    rng = np.random.default_rng(seed)
    perm = rng.permutation(len(items))[:n]
    return [items[i] for i in perm]

def build_loaders(data_dir: Path, task: str, fraction: int, seed: int, batch_size=2, num_workers=2):
    all_items = load_decathlon_list(data_dir, task, download=True)
    # fixed val split
    train_pool, val_items = train_val_split(all_items, DEFAULT_VAL_FRAC, seed=0)  # seed=0 to keep "test" fixed across runs
    ...
    train_eval_loader = DataLoader(train_eval_ds, batch_size=1, shuffle=False, num_workers=num_workers, pin_memory=True)

    # Save split for reproducibility
    return train_loader, val_loader, train_eval_loader, {"train": train_items, "val": val_items}
